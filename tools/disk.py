...
def retrieve_all_fcd(content, datastore):
    """
    Retrieves the managed object for the first class disk specified
    ...
    # Set vStorageObjectManager
    storage = content.vStorageObjectManager
    storid = storage.ListVStorageObject(datastore)
    
    for d in storid:
        vdisk_name = storage.RetrieveVStorageObject(d, datastore)
        print("all about this vdisk:", vdisk_name.config.name)
        print(vdisk_name)
